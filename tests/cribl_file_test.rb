...
class CriblFileTest < Test::Unit::TestCase
  ...
  def test_exists?
    ...
    conditions.each do |c|
      f = Services::CriblFile.new(TEST_LOGS, {:filename => c[0]})
      assert_equal(c[1], f.exists?, c[2])
    end
  end

  def test_read
    f = Services::CriblFile.new(TEST_LOGS, {:filename => 'sample-1.log', :lines => 2})
    original = [
  ...
  def test_query_with_nocase
    f = Services::CriblFile.new(TEST_LOGS, {:filename => 'sample-1.log', :q => 'mailBOX_register'})

    assert_equal 4, f.process.size
  end

  def test_query_with_case
    f = Services::CriblFile.new(TEST_LOGS, {:filename => 'sample-1.log', :q => 'mailBOX_register', :ignore_case => :false})

    assert_equal 0, f.process.size
  end

  def test_last_new_lines
    f = Services::CriblFile.new(TEST_LOGS, {:filename => 'test.log', :lines => 1})
    data = f.process
  ...
  def test_ensure_all_chars_show_up
    f = Services::CriblFile.new(TEST_LOGS, {:filename => 'test.log'})
    data = f.process
